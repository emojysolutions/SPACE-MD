...
/**
 * Sonnet Command - Chat with Claude Sonnet
 * Usage: !sonnet <message> or !sonnet clear
 */
module.exports = {
  name: 'sonnet',
  ...
  async execute(sock, msg, args) {
    ...
    if (!aiManager.isProviderAvailable('sonnet')) {
      await sock.sendMessage(userId, {
        text: '❌ *Claude Sonnet is not configured*\n\nAsk the bot admin to add `ANTHROPIC_API_KEY` to the .env file.'
      });
      return;
    }
    ...
    if (args.length === 1 && args[0].toLowerCase() === 'clear') {
      aiManager.clearHistory(senderNumber, 'sonnet');
      await sock.sendMessage(userId, {
        text: '✅ *Claude Sonnet conversation history cleared!*\n\nStart a fresh conversation with !sonnet <message>'
      });
      return;
    }
    ...
    if (args.length === 0) {
      await sock.sendMessage(userId, {
        text: '❌ *Please include a message!*\n\nExample: `!sonnet write a haiku about coding`'
      });
      return;
    }
    ...
    try {
      ...
      const response = await aiManager.chat('sonnet', senderNumber, message, systemPrompt);

      // Format response
      const provider = aiManager.getProvider('sonnet');
      ...
      formattedResponse += '━'.repeat(24) + '\n';
      formattedResponse += '💬 _Reply with !sonnet to continue_';
