...
/**
 * DeepSeek Command - Chat with DeepSeek
...
module.exports = {
  ...
  async execute(sock, msg, args) {
    ...
    if (!aiManager.isProviderAvailable('deepseek')) {
      await sock.sendMessage(userId, {
        text: '❌ *DeepSeek is not configured*\n\nAsk the bot admin to add `DEEPSEEK_API_KEY` to the .env file.'
      });
      return;
    }
    ...
    if (args.length === 1 && args[0].toLowerCase() === 'clear') {
      aiManager.clearHistory(senderNumber, 'deepseek');
      await sock.sendMessage(userId, {
        text: '✅ *DeepSeek conversation history cleared!*\n\nStart a fresh conversation with !deepseek <message>'
      });
      return;
    }
    ...
    try {
      ...
      const response = await aiManager.chat('deepseek', senderNumber, message, systemPrompt);

      // Format response
      const provider = aiManager.getProvider('deepseek');
