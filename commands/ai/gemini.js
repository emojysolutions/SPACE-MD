...
/**
 * Gemini Command - Chat with Google Gemini Pro
  ...
  async execute(sock, msg, args) {
    ...
    if (!aiManager.isProviderAvailable('gemini')) {
      await sock.sendMessage(userId, {
        text: '❌ *Google Gemini is not configured*\n\nAsk the bot admin to add `GOOGLE_AI_KEY` to the .env file.'
      });
      return;
    }
    ...
    if (args.length === 1 && args[0].toLowerCase() === 'clear') {
      aiManager.clearHistory(senderNumber, 'gemini');
      await sock.sendMessage(userId, {
        text: '✅ *Gemini conversation history cleared!*\n\nStart a fresh conversation with !gemini <message>'
      });
      return;
    }
    ...
    if (args.length === 0) {
      await sock.sendMessage(userId, {
        text: '❌ *Please include a message!*\n\nExample: `!gemini what\'s the weather like on Mars?`'
      });
      return;
    }
    ...
    try {
      ...
      const response = await aiManager.chat('gemini', senderNumber, message, systemPrompt);
